...
class RegistrationForm extends User {
	...
	public function rules() {
		$rules = array(
			...
			array('username', 'length', 'max'=>20, 'min' => 4, 'message' => UserModule::t("Nombre de usuario Incorecto (length between 4 and 20 characters).")),
			array('password', 'length', 'max'=>128, 'min' => 8, 'message' => UserModule::t("Incorrect password (minimal length 8 symbols).")),
			array('password', 'match', 'pattern'=>'/^.*(?=.{8,})(?=.*\d).*$/', 'message' => UserModule::t("Password must contain at least one digit, one uppercase letter and one lowercase letter.")),
			#array('password', 'match', 'message' => UserModule::t("Password must contain at least one digit, one uppercase letter and one lowercase letter.")),
			array('email', 'email'),
